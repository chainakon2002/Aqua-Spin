...
function App() {
  ...
  useEffect(() => {
    // Attempt to start audio immediately (may be blocked by browser policy)
    const tryInitAudio = () => {
      initAudio();
      // Remove listeners once clicked
    ...
    // Fallback: any interaction will start the audio transparently
  ...
  return (
    <>
      ...
      <div className="md:hidden fixed inset-0 bg-sky-500 z-[9999] flex flex-col items-center justify-center p-8 text-center text-white">
        <MonitorSmartphone size={80} className="mb-6 opacity-80" />
        ...
        <p className="mt-6 text-white/80 bg-white/10 px-6 py-3 rounded-full text-sm">
          โปรดเข้าใช้งานผ่านอุปกรณ์หน้าจอใหญ่เพื่อเริ่มเล่น
        </p>
      </div>

      {/* Main Game Container */}
      <div className="hidden md:flex w-full min-h-screen bg-sky-50 items-center justify-center p-4 font-sans relative">
        <button
          onClick={goHome}
          className="absolute top-4 left-4 z-50 bg-white/80 hover:bg-white text-sky-600 font-bold px-4 py-2 rounded-full shadow-md flex items-center gap-2 transition-all hover:scale-105"
        >
